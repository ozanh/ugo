...
func TestObjectCallable(t *testing.T) {
	...
	v, err := Int(0).Call(nil)
	require.Nil(t, v)
	require.Equal(t, ErrNotCallable, err)
	v, err = Uint(0).Call(nil)
	require.Nil(t, v)
	require.Equal(t, ErrNotCallable, err)
	v, err = Char(0).Call(nil)
	require.Nil(t, v)
	require.Equal(t, ErrNotCallable, err)
	v, err = Float(0).Call(nil)
	require.Nil(t, v)
	require.Equal(t, ErrNotCallable, err)
	v, err = Bool(true).Call(nil)
	require.Nil(t, v)
	require.Equal(t, ErrNotCallable, err)
	require.Equal(t, ErrNotCallable, err)
	v, err = (&Error{}).Call(nil)
	require.Nil(t, v)
	require.Equal(t, ErrNotCallable, err)
	v, err = (&RuntimeError{}).Call(nil)
	require.Nil(t, v)
	require.Equal(t, ErrNotCallable, err)
	v, err = String("").Call(nil)
	require.Nil(t, v)
	require.Equal(t, ErrNotCallable, err)
	v, err = Array{}.Call(nil)
	require.Nil(t, v)
	require.Equal(t, ErrNotCallable, err)
	v, err = Bytes{}.Call(nil)
	require.Nil(t, v)
	require.Equal(t, ErrNotCallable, err)
	v, err = Map{}.Call(nil)
	require.Nil(t, v)
	require.Equal(t, ErrNotCallable, err)
	v, err = (&SyncMap{}).Call(nil)
	require.Nil(t, v)
	require.Equal(t, ErrNotCallable, err)
}
...
func TestObjectCopier(t *testing.T) {
	...
	for _, o := range objects {
		if _, ok := o.(DeepCopier); !ok {
			t.Fatalf("%T must implement DeepCopier interface", o)
		}
	}
}

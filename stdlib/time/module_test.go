...
func TestModuleDuration(t *testing.T) {
	...
	_, err := durToString.Call(nil)
	require.Error(t, err)

	durParse := Module["ParseDuration"].(*Function)
	_, err = durParse.Call(nil)
	require.Error(t, err)
	_, err = durParse.Call(nil, String(""))
	require.Error(t, err)
	_, err = durParse.Call(nil, Int(0))
	require.NoError(t, err)
	...
	for _, tC := range testCases {
		for fn := range goFnMap {
			t.Run(fmt.Sprintf("%s:%s", tC.dur, fn), func(t *testing.T) {
				...
				ret, err := f.Call(nil, Int(tC.dur))
				require.NoError(t, err)
				expect := goFnMap[fn](tC.dur)
				require.EqualValues(t, expect, ret)

				// test illegal type
				_, err = f.Call(nil, &illegalDur{Value: tC.dur})
				require.Error(t, err)
				// test no arg
				_, err = f.Call(nil)
				require.Error(t, err)
				...
				// test to string
				s, err := durToString.Call(nil, Int(tC.dur))
				require.NoError(t, err)
				require.EqualValues(t, tC.dur.String(), s)

				// test parse
				d, err := durParse.Call(nil, s)
				require.NoError(t, err)
	...
	r, err := durRound.Call(nil, Int(time.Second+time.Millisecond),
		Int(time.Second))
	require.NoError(t, err)
	require.EqualValues(t, time.Second, r)
	_, err = durRound.Call(nil, Int(0))
	require.Error(t, err)
	_, err = durRound.Call(nil, String(""), Int(0))
	require.Error(t, err)
	_, err = durRound.Call(nil, Int(0), String(""))
	require.Error(t, err)

	durTruncate := Module["DurationTruncate"].(*Function)
	r, err = durTruncate.Call(nil, Int(time.Second+5*time.Millisecond),
		Int(2*time.Millisecond))
	require.NoError(t, err)
	require.EqualValues(t, time.Second+4*time.Millisecond, r)
	_, err = durTruncate.Call(nil, Int(0))
	require.Error(t, err)
	_, err = durTruncate.Call(nil, String(""), Int(0))
	require.Error(t, err)
	_, err = durTruncate.Call(nil, Int(0), String(""))
	require.Error(t, err)
}

func TestModuleLocation(t *testing.T) {
	fixedZone := Module["FixedZone"].(*Function)
	r, err := fixedZone.Call(nil, String("Ankara"), Int(3*60*60))
	require.NoError(t, err)
	require.Equal(t, "Ankara", r.String())

	_, err = fixedZone.Call(nil, String("Ankara"))
	require.Error(t, err)
	_, err = fixedZone.Call(nil, String("Ankara"), Uint(0))
	require.NoError(t, err)
	_, err = fixedZone.Call(nil, Int(0), Array{})
	require.Error(t, err)
	_, err = fixedZone.Call(nil)
	require.Error(t, err)

	loadLocation := Module["LoadLocation"].(*Function)
	r, err = loadLocation.Call(nil, String("Europe/Istanbul"))
	require.NoError(t, err)
	require.Equal(t, "Europe/Istanbul", r.String())
	r, err = loadLocation.Call(nil, String(""))
	require.NoError(t, err)
	require.Equal(t, "UTC", r.String())
	_, err = loadLocation.Call(nil)
	require.Error(t, err)
	_, err = loadLocation.Call(nil, Int(0))
	require.Error(t, err)
	_, err = loadLocation.Call(nil, String("invalid"))
	require.Error(t, err)

	isLocation := Module["IsLocation"].(*Function)
	r, err = isLocation.Call(nil, &Location{Value: time.Local})
	require.NoError(t, err)
	require.EqualValues(t, true, r)
	r, err = isLocation.Call(nil, Int(0))
	require.NoError(t, err)
	require.EqualValues(t, false, r)
	_, err = isLocation.Call(nil, Int(0), Int(0))
	require.Error(t, err)
	_, err = isLocation.Call(nil)
	require.Error(t, err)
}
...
func TestModuleTime(t *testing.T) {
	...
	r, err := zTime.Call(nil)
	require.NoError(t, err)
	require.True(t, r.(*Time).Value.IsZero())
	_, err = zTime.Call(nil, String(""))
	require.Error(t, err)

	since := Module["Since"].(*Function)
	r, err = since.Call(nil, &Time{Value: now})
	require.NoError(t, err)
	require.GreaterOrEqual(t, int64(r.(Int)), int64(0))
	_, err = since.Call(nil)
	require.Error(t, err)
	_, err = since.Call(nil, String(""))
	require.Error(t, err)

	until := Module["Until"].(*Function)
	r, err = until.Call(nil, &Time{Value: now})
	require.NoError(t, err)
	require.LessOrEqual(t, int64(r.(Int)), int64(0))
	_, err = until.Call(nil)
	require.Error(t, err)
	_, err = until.Call(nil, String(""))
	require.Error(t, err)

	date := Module["Date"].(*Function)
	r, err = date.Call(nil, Int(2020), Int(11), Int(8),
	...
	_, err = nowf.Call(nil, Int(0))
	require.Error(t, err)

	RFC3339Nano := Module["RFC3339Nano"]
	parse := Module["Parse"].(*Function)
	r, err = parse.Call(nil, RFC3339Nano, String(now.Format(time.RFC3339Nano)))
	require.NoError(t, err)
	...
	_, err = parse.Call(nil)
	require.Error(t, err)

	unix := Module["Unix"].(*Function)
	r, err = unix.Call(nil, Int(now.Unix()))
	require.NoError(t, err)
	require.Equal(t, time.Unix(now.Unix(), 0), r.(*Time).Value)
	r, err = unix.Call(nil, Int(now.Unix()), Int(1))
	require.NoError(t, err)
	...
	r, err = add.Call(nil, &Time{Value: now}, Int(time.Second))
	require.NoError(t, err)
	require.Equal(t, now.Add(time.Second), r.(*Time).Value)
	_, err = add.Call(nil, &Time{Value: now})
	require.Error(t, err)
	_, err = add.Call(nil, &Time{Value: now}, &Time{Value: now})
	require.Error(t, err)
	_, err = add.Call(nil)
	require.Error(t, err)

	sub := Module["Sub"].(*Function)
	r, err = sub.Call(nil, &Time{Value: now}, &Time{Value: now.Add(-time.Hour)})
	require.NoError(t, err)
	require.EqualValues(t, time.Hour, r.(Int))
	_, err = sub.Call(nil, &Time{Value: now})
	require.Error(t, err)
	_, err = sub.Call(nil, &Time{Value: now}, Int(0))
	require.NoError(t, err)
	_, err = sub.Call(nil)
	require.Error(t, err)

	addDate := Module["AddDate"].(*Function)
	r, err = addDate.Call(nil, &Time{Value: now},
		Int(1), Int(2), Int(3))
	require.NoError(t, err)
	require.EqualValues(t, now.AddDate(1, 2, 3), r.(*Time).Value)
	_, err = addDate.Call(nil, &Time{Value: now})
	require.Error(t, err)
	_, err = addDate.Call(nil, &Time{Value: now}, Int(0))
	require.Error(t, err)
	_, err = addDate.Call(nil)
	require.Error(t, err)

	after := Module["After"].(*Function)
	r, err = after.Call(nil, &Time{Value: now}, &Time{Value: now.Add(time.Hour)})
	require.NoError(t, err)
	require.EqualValues(t, false, r)
	r, err = after.Call(nil, &Time{Value: now}, &Time{Value: now.Add(-time.Hour)})
	require.NoError(t, err)
	require.EqualValues(t, true, r)
	_, err = after.Call(nil, &Time{Value: now}, Int(0))
	require.NoError(t, err)
	_, err = after.Call(nil, &Time{Value: now})
	require.Error(t, err)
	_, err = after.Call(nil)
	require.Error(t, err)

	before := Module["Before"].(*Function)
	r, err = before.Call(nil, &Time{Value: now}, &Time{Value: now.Add(time.Hour)})
	require.NoError(t, err)
	require.EqualValues(t, true, r)
	r, err = before.Call(nil, &Time{Value: now}, &Time{Value: now.Add(-time.Hour)})
	require.NoError(t, err)
	require.EqualValues(t, false, r)
	_, err = before.Call(nil, &Time{Value: now}, Int(0))
	require.NoError(t, err)
	_, err = before.Call(nil, &Time{Value: now})
	require.Error(t, err)
	_, err = before.Call(nil)
	require.Error(t, err)

	appendFormat := Module["AppendFormat"].(*Function)
	b := make(Bytes, 100)
	r, err = appendFormat.Call(nil, &Time{Value: now}, b, RFC3339Nano)
	require.NoError(t, err)
	require.EqualValues(t,
		now.AppendFormat(make([]byte, 100), time.RFC3339Nano), r)
	_, err = appendFormat.Call(nil, &Time{Value: now}, b)
	require.Error(t, err)
	_, err = appendFormat.Call(nil, &Time{Value: now})
	require.Error(t, err)
	_, err = appendFormat.Call(nil)
	require.Error(t, err)

	format := Module["Format"].(*Function)
	r, err = format.Call(nil, &Time{Value: now}, RFC3339Nano)
	require.NoError(t, err)
	require.EqualValues(t, now.Format(time.RFC3339Nano), r)
	_, err = format.Call(nil, &Time{Value: now})
	require.Error(t, err)
	_, err = format.Call(nil)
	require.Error(t, err)

	timeIn := Module["In"].(*Function)
	r, err = timeIn.Call(nil, &Time{Value: now}, &Location{Value: time.Local})
	require.NoError(t, err)
	require.False(t, r.(*Time).Value.IsZero())
	_, err = timeIn.Call(nil, &Time{Value: now})
	require.Error(t, err)
	_, err = timeIn.Call(nil)
	require.Error(t, err)

	round := Module["Round"].(*Function)
	r, err = round.Call(nil, &Time{Value: now}, Int(time.Second))
	require.NoError(t, err)
	require.Equal(t, now.Round(time.Second), r.(*Time).Value)
	_, err = round.Call(nil, &Time{Value: now})
	require.Error(t, err)
	_, err = round.Call(nil)
	require.Error(t, err)

	truncate := Module["Truncate"].(*Function)
	r, err = truncate.Call(nil, &Time{Value: now}, Int(time.Hour))
	require.NoError(t, err)
	require.Equal(t, now.Truncate(time.Hour), r.(*Time).Value)
	_, err = truncate.Call(nil, &Time{Value: now})
	require.Error(t, err)
	_, err = truncate.Call(nil)
	require.Error(t, err)

	isTime := Module["IsTime"].(*Function)
	r, err = isTime.Call(nil, &Time{Value: now})
	require.NoError(t, err)
	require.EqualValues(t, true, r)
	r, err = isTime.Call(nil, Int(0))
	require.NoError(t, err)
	require.EqualValues(t, false, r)
	_, err = isTime.Call(nil, Int(0), Int(0))
	require.Error(t, err)
	_, err = isTime.Call(nil)
	require.Error(t, err)
